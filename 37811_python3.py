...
url = re.search("ajaxBlockUrl = '(.*)'".encode('utf-8'), content)
url = url.group(1)
key = re.search("var FORM_KEY = '(.*)'".encode('utf-8'), content)
key = key.group(1)
...
request = br.open(url + 'block/tab_orders/period/7d/?isAjax=true', data='isAjax=false&form_key=' + key)
tunnel = re.search("src=\"(.*)\?ga=".encode('utf-8'), request.read())
